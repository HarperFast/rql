...
Query.prototype.walk = function(fn, options){
	...
	function walk(){
		...
		this.args = this.args.map(function(term, i, arr) {
			...
			var newTerm = f.call(term);
			console.log('NEWTERM', newTerm);
			return newTerm;
			if (typeof newTerm == 'undefined')
				return undefined;
				//delete arr[i];
				//arr.splice(i, 1);
				//return {};
	...
	//q.args = q.args.filter(function(x){return x;});
	console.log('NORMAL', q);
};
...
Query.prototype.normalize = function(options){
	...
	function normal(){
		var func = this.name;
		var args = this.args;
		console.log('NORMALIZE?', func, args, this);
		if (!func || !args) return;
		...
		if (func === 'sort' || func === 'select') {
			...
			// TODO: limit the topterm level only!
	...
	//result.original = this.walk(normal).filter(function(x){return x;});
	this.walk(normal);
	return result;
};
