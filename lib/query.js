...
Query.prototype.normalize = function(options){
	...
	// define morpher
	result.toString = function(overrides){
		overrides = overrides || {};
		//var self = this;
		var q = this.search;
		['sort', 'select', 'limit', 'values'].forEach(function(op){
			var args = overrides[op] === undefined ? overrides[op] || result[op] : overrides[op];
			if (args instanceof Array && args.length)
				q = q[op].apply(q, args);
		});
		console.log('MORPHED', result, q+'');
		return q.toString();
	};
