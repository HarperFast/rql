	...
	var leftoverCharacters = query.replace(/(\))|([&\|,])?([\+\*\$\-:\w%\._]*)(\(?)/g,
		...
		function(t, closedParan, delim, propertyOrValue, openParan){
			...
			if(openParan){
				...
				// cache the last seen id equality. N.B. only top level one is valid
				if (term.name === 'eq' && term.args[0] === exports.primaryKeyName && term.parent === topTerm) {
					var id = term.args[1];
					// FIXME: safe to punt on RegExps here now?
					//if (id && !(id instanceof RegExp)) id = String(id);
					topTerm.cache.pk = String(id);
				}
			}
			return "";
		});
